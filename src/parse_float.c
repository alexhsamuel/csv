...
static int parser(char const* s, struct PrepNumber *pn)
{
  int state = FSM_A;
  ...
  while (state != FSM_STOP) {
    ...
    case FSM_B:
      // Read optional sign, continue to state C.
      state = FSM_C;
      if (c == '+') 
        c = GETC(s);
      else if (c == '-') {
  ...
  if (pn->mantissa == 0)
    result = pn->negative ? PARSER_MZERO : PARSER_PZERO;
  else if (pn->exponent > 309)
    result = pn->negative ? PARSER_MINF : PARSER_PINF;
